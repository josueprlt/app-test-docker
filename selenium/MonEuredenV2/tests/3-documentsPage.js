...
async function DocumentsPage() {
    const type = "3-documentsPage";
    await waitForSelenium();
    await createTest(
        "Parcours vers les Documents",
        "Vérifie le parcours utilisateur permettant de se rendre jusqu'à la page des documents par rapport à l'application MonEureden V2",
        type
    );
    ...
    try {
        ...
        await executeStep(
            () => navigationStep(driver, 'Documents'),
            '✅ Clic sur la page document réussie',
            '❌ Échec du clic sur la page document',
            'Frontend',
            type
        );
