...
import { IconCheck, IconCross, IconWarn } from "../components/icons";
import { Link } from "react-router";

const TestsAffichage = ({ filteredData = null, ChangeInfoTest, clickedElt, setClickedElt }) => {
    ...
    return (
        <section className="flex gap-2 w-full mt-10 gap-2 py-2 rounded-md bg-gray-500 overflow-x-auto xl:gap-6">
            {filtreData && clickedElt && (
                filtreData.map((dt, index) => (
                    <div
                        onClick={() => ChangeInfoTest(dt.id)}
                        key={index}
                        className={`flex items-center gap-1 px-1 py-2 rounded-md xl:gap-6 ${clickedElt === dt.id ? classElt.actif : classElt.notActif}`}
                    >
                        {dt.success === true ? (
                        ...
                        {dt.extractedValues.map((ev, idx) => {
                            ...
                            return (
                                <div key={idx} className="flex gap-1 xl:gap-6">
                                    <span>{ev}</span>
                                    <span>{!isLast && "/"}</span>
                                </div>
                            );
