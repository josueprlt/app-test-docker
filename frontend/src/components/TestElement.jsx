...
const TestElement = ({ id, index, title, date, success, logs }) => {
    ...
    return (
        <Link to={`/test/${id}`} className={`relative flex justify-between gap-4 h-[80px] rounded-md overflow-hidden bg-gray-500 cursor-pointer before:w-[75%] before:h-full before:absolute before:top-0 before:right-0 before:bg-linear-to-r before:from-gray-transparent-500 before:z-10 sm:h-[100px] hover:bg-gray-300 ${bgElt}`}>
                ...
                <div className="hidden items-center gap-2 md:flex">
                    ...
                    {success === null && (
                        <div className="flex flex-row items-center gap-3">
                            ...
                            <p className="text-orange-500">En cours d’éxécution...</p>
                        </div>
                    )}
                </div>
